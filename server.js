...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/signup' && req.method === 'POST') {
    try {
      ...
      if (!name || !phone || !['customer', 'transporter'].includes(role)) {
  ...
  if (url.pathname === '/api/requests' && req.method === 'POST') {
    ...
    try {
      ...
      if (!from || !to || amount === undefined || amount === null || Number(amount) <= 0) {
...
server.listen(PORT, () => {
  ...
  console.log(`Student Delivery Network running at http://localhost:${PORT}`);
});
